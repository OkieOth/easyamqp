use easyamqp::utils::{get_env_var_int, get_env_var_str};
...
fn extract_conn_name_from_json(conn_json_str: &str, connection_name: &str) -> Option<String> {
    ...
    let json_path =
        JsonPath::parse(json_path_str.as_str()).expect("error while construct json_path");
    ...
    for i in 0..count_of_kills {
        ...
        loop {
            match get_conn_name(
                ...
                None => {
                    println!(
                        "sorry, can't find connection to close ... sleep for {}s, attempt {}/{}",
                        reconnect_seconds, reconnect_attempts, max_reconnect_attempts
                    );
        ...
        println!(
            "connection closed {}/{}, waiting for next try ...",
            i, count_of_kills
        );
